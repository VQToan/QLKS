...
public class mainFrame extends JFrame {
	...
	public mainFrame() {
		...
		jLabel5.setText("4. Trần Minh Hoàng");

		JLabel jLabel6 = new JLabel();
		jLabel6.setText("5. Võ Quốc Toàn");
		GroupLayout gl_jPInforTeam = new GroupLayout(jPInforTeam);
		gl_jPInforTeam.setHorizontalGroup(gl_jPInforTeam.createParallelGroup(Alignment.LEADING)
				.addGap(0, 1332, Short.MAX_VALUE)
				.addGroup(gl_jPInforTeam.createSequentialGroup().addGap(17)
		...
		gl_jPInforTeam.setVerticalGroup(gl_jPInforTeam.createParallelGroup(Alignment.LEADING)
				.addGap(0, 808, Short.MAX_VALUE)
				.addGroup(gl_jPInforTeam.createSequentialGroup().addGap(18)
						.addComponent(jLabel2, GroupLayout.PREFERRED_SIZE, 24, GroupLayout.PREFERRED_SIZE).addGap(18)
						.addComponent(jLabel3, GroupLayout.PREFERRED_SIZE, 24, GroupLayout.PREFERRED_SIZE).addGap(18)
						.addComponent(jLabel4, GroupLayout.PREFERRED_SIZE, 24, GroupLayout.PREFERRED_SIZE).addGap(18)
						.addComponent(jLabel5, GroupLayout.PREFERRED_SIZE, 24, GroupLayout.PREFERRED_SIZE).addGap(18)
		...
		JLabel lblTypeRent1 = new JLabel("Sắp xếp:");
		...
		cbbTypeRent1.setModel(new DefaultComboBoxModel<String>(new String[] { "", "Tăng dần theo giá thuê giờ",
				"Tăng dần theo giá thuê qua đêm", "Giảm dần theo giá thuê giờ", "Giảm dần theo giá thuê qua đêm" }));
		...
		btnSearchRoom.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				listEndSearchRooms = search.SearchRoom(listRooms, txtIDsRoom.getText(),
						cbbStatus.getItemAt(cbbStatus.getSelectedIndex()),
						cbbBeds.getItemAt(cbbBeds.getSelectedIndex()),
						cbbTypeRoom.getItemAt(cbbTypeRoom.getSelectedIndex()).toString(), txtPrice.getText());
				// listEndSearchRooms= search.SearchRoom(listRooms, "", "",
				// cbbBeds.getItemAt(cbbBeds.getSelectedIndex()), "", "");
				ResetTableRoom();
				System.out.println(listEndSearchRooms.get(0).toString());
				System.out.println(cbbBeds.getSelectedItem());
				System.out.println(cbbStatus.getSelectedItem());
				System.out.println(cbbTypeRent1.getSelectedItem());
				System.out.println(cbbTypeRoom.getSelectedItem());
				if (cbbTypeRent1.getSelectedIndex() == 1) {
					Collections.sort(listEndSearchRooms, new Comparator<Room>() {
						@Override
						public int compare(Room r1, Room r2) {
							return (r1.getPrice1Hour() > r2.getPrice1Hour()) ? 1
									: (r1.getPrice1Hour() == r2.getPrice1Hour()) ? 0 : -1;
						}
					});
				}
				if (cbbTypeRent1.getSelectedIndex() == 2) {
					Collections.sort(listEndSearchRooms, new Comparator<Room>() {
						@Override
						public int compare(Room r1, Room r2) {
							return (r1.getPriceOverNight() > r2.getPriceOverNight()) ? 1
									: (r1.getPriceOverNight() == r2.getPriceOverNight()) ? 0 : -1;
						}
					});
		...
		cbbTimeIn.setModel(new DefaultComboBoxModel<String>(new String[] { "", "0:00 AM", "1:00 AM", "2:00 AM",
		...
		cbbTimeOut.setModel(new DefaultComboBoxModel<String>(new String[] { "", "0:00 AM", "1:00 AM", "2:00 AM",
				"3:00 AM", "4:00 AM", "5:00 AM", "6:00 AM", "7:00 AM", "8:00 AM", "9:00 AM", "10:00 AM", "11:00 AM",
				"12:00 AM", "1:00 PM", "2:00 PM", "3:00 PM", "4:00 PM", "5:00 PM", "6:00 PM", "7:00 PM", "8:00 PM",
				"9:00 PM", "10:00 PM", "11:00 PM" }));
		...
		btnSearchCustomer.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				listInforSearchArrayList = search.searchCustomers(listCustomers, txtID.getText(), txtPhone.getText(),
						cbbTypeRent.getSelectedItem().toString(), txtFullName.getText(), txtIDPersonal.getText(),
						txtIDRoom.getText(), txtDayIn.getText(), txtDayOut.getText(),
						convertTime(cbbTimeIn.getSelectedItem().toString()),
						convertTime(cbbTimeOut.getSelectedItem().toString()));
				resetTableCus();
//				System.out.println(convertTime(cbbTimeIn.getSelectedItem().toString()));
				...
				if (listInforSearchArrayList.get(0).getRollNo().equals("NoneItem")) {
					...
					ShowOnTableCus(listInforSearchArrayList, listRooms);
				}
		...
		jPRegister.add(txtTime);
		jPRegister.add(jLabel17);
		...
		cbbTypeRent2.setModel(new DefaultComboBoxModel<>(new String[] {""}));
		...
		cbbTypeRoom2.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				cbbBeds2.setModel(new DefaultComboBoxModel<>(setOptionBeds(roomSuggest)));
				roomSuggest = search.SearchRoom(listRooms, "", "Trống", cbbBeds2.getSelectedItem().toString(),
						cbbTypeRoom2.getSelectedItem().toString(), "");
				cbbIDRoom2.setModel(new DefaultComboBoxModel<>(setOptionID(roomSuggest)));
				if (txtTime.getText().equals("")) {
					if (cbbTypeRent2.getSelectedItem().equals("")) {
		...
		cbbTypeRent2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (txtTime.getText().equals("")) {
					JOptionPane.showMessageDialog(panel2, "Vui lòng nhập số giờ vào ô thời gian thuê");
				}
				else{
					int value= Integer.parseInt(txtTime.getText().toString());
					if(!cbbTypeRent2.getSelectedItem().equals(""))
		...
		txtTime.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				else if (!isNumeric(txtTime.getText())) {
					JOptionPane.showMessageDialog(panel2, "Vui lòng chỉ nhập số vào ô thời gian thuê");
					txtTime.setText("");
				}else{
					cbbTypeRent2.setModel(new DefaultComboBoxModel<>(new String[] {""}));
					if (cbbTypeRent2.getSelectedItem().equals("")) {
						...
						String dateIn = simplpDateFormat.format(date);
						calendar.add(Calendar.HOUR_OF_DAY, Integer.parseInt(time));
						date = calendar.getTime();
						String dateIn1 = simplpDateFormat.format(date);
						if (dateIn.equals(dateIn1)) {
							cbbTypeRent2.addItem("Theo giờ");
							cbbTypeRent2.setSelectedIndex(1);
						} else{
							cbbTypeRent2.addItem("Qua đêm");
							cbbTypeRent2.setSelectedIndex(1);				
						}					
					}
				}				
		...
		btnRegister.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				String check=checkInputReg(txtFullName2.getText(),txtIDPersonal2.getText(), txtPhone2.getText(),cbbIDRoom2.getItemAt(cbbIDRoom2.getSelectedIndex()));
					...
					if (check.equals("")) {
						if (txtTime.getText().equals("")) {
							JOptionPane.showMessageDialog(panel2, "Vui lòng nhập vào ô thời gian thuê");
						} else if(!isNumeric(txtTime.getText())){
							JOptionPane.showMessageDialog(panel2, "Vui lòng chỉ nhập số vào ô thời gian thuê");		
						}else {
							...
							temp.setiDRoom(cbbIDRoom2.getSelectedItem().toString());
							temp.setTypeRent(cbbTypeRent2.getSelectedItem().toString());
							temp.setInOut(action.TotalTime(txtTime.getText().toString()));
							temp.setRollNo(rollNo);
							temp.Cost(txtTime.getText().toString());
							hashMapCustHashMap.put(rollNo, temp);
							listCustomers.add(temp);
							input.exportCustomer(listCustomers);
							String iDRoomString= cbbIDRoom2.getSelectedItem().toString();
							...
							JOptionPane.showMessageDialog(panel1,  "ID của bạn: "+rollNo+"\nID phòng bạn là: "+iDRoomString+
																	"\nLoại phòng: "+cbbTypeRoom2.getSelectedItem().toString()+
																	"\nSố giường: "+cbbBeds2.getSelectedItem().toString()+
																	"\nHình thức thuê: "+temp.getTypeRent()+
																	"\nTổng số tiền: "+temp.getPayment()
         ...
         for (int i=0; i< listCustomers.size();i++) {
         	Object[] data= {i+1,
							  ...
							  listCustomers.get(i).getTypeRent(),
         					  listCustomers.get(i).getPayment()};
         	tableModel.addRow(data);
         	}
         tableCust.setModel(tableModel);
		}
     ...
     public String createRollNo(HashMap<String, Customer> dataHashMap) {
		String endString="";
		int id=listCustomers.size()+1;
		if(dataHashMap.containsKey("KH"+String.valueOf(id))) {}
		else return endString="KH"+String.valueOf(id);		
		return endString;
     }
